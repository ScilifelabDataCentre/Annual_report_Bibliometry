...
fellpubs_jif_count.columns = ["Year", "Category", "Count"]

fellpubs_ais_count = (
    pubs_jif_ais_info.groupby(["Year", "AIS_category"]).size().reset_index()
)
fellpubs_ais_count.columns = ["Year", "Category", "Count"]
...
with pd.ExcelWriter("fellows_publications_JIF_AIS_counts.xlsx") as out_excel:
    fellpubs_jif_count.to_excel(out_excel, sheet_name="JIF", index=False)
    fellpubs_ais_count.to_excel(out_excel, sheet_name="AIS", index=False)
