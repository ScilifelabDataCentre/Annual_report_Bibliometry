...
# need to keep papers from 2013 because we count citations from papers published
Affiliates_data = Affiliates_data[
    (Affiliates_data["Publication_year"] > 2012)
    & (Affiliates_data["Publication_year"] < 2024)
]
...
Affiliates_data_sub = Affiliates_data[
    [
        ...
        "citations_self_excl_2022",
        "citations_self_excl_2023",
    ]
]
...
Infra_data = Infra_data[
    (Infra_data["Publication_year"] > 2012) & (Infra_data["Publication_year"] < 2024)
]
...
def Citation_graph_func(input, pub_group):
    ...
    if not os.path.isdir("Plots/"):
        os.mkdir("Plots/")
    fig.show()
    fig.write_image("Plots/{}_Citation_eachyear_since2018.png".format(pub_group))
    fig.write_image("Plots/{}_Citation_eachyear_since2018.svg".format(pub_group))
