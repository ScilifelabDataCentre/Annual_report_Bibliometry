# For the annual report, need the last 5 years for all groups (2017-2021 in this case)
import pandas as pd
import os
...
def JIF_graph_func(input, name):
    ...
    fig.update_xaxes(
        ...
        linecolor="black",
        # years
        ticktext = [ "<b>" + Years[i] + "</b>" for i in range(len(Years_int))  ],

        tickvals=[ Years[i] for i in range(len(Years_int)) ],
    )

    Year_sums = [ JIFcounts[(JIFcounts["Year"] == Years_int[i])] for i in range(len(Years_int)) ]

    highest_y_value = max( [ Year_sums[i]["Count"].sum() for i in range(len(Years_int)) ] )
